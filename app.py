...
def add_player(name, email, phno, age):
    con = sqlite3.connect("Elitmus2.db")
    c = con.cursor()
    try:
        x = (name, email, phno, age, 0, 0, 0)
        insert = """INSERT INTO Demo(name, email, phno, age, start, stop, marks) VALUES (?,?,?,?,?,?,?);"""
        c.execute(insert, x)
    finally:
        con.commit()
        c.close()
...
def get_new_player():
    ...
    c = con.cursor()
    try:
        sql_select_query = "select max(userid) from Demo;"
...
def get_player(id):
    ...
    c = con.cursor()
    try:
        sql_select_query = "select * from Demo where userid = ?;"
...
def update_player():
    ...
    try:
        ...
        update = "UPDATE Demo SET start = ?, stop = ?, marks = ? WHERE userid = ?;"
        c.execute(update, x)
    finally:
        con.commit()
        c.close()
...
def get_players_played():
    ...
    c = con.cursor()
    try:
        sql_select_query = "select * from Demo where start > 0 and stop > 0;"
        c.execute(sql_select_query)
        record = c.fetchall()
    finally:
        con.commit()
...
def login():
    if request.method == "POST":
        req = request.form
        if req["email"] == "Admin@gmail.com" and req["password"] == "1234":
